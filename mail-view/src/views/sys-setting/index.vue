        ...
        <div class="card-grid">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div v-if="superAdmin" class="setting-item site-icon-setting">
                ...
                <div class="site-icon-actions">
                  <div class="site-icon-preview" :class="{ empty: !siteIconUrl }">
                    <img
                      v-if="siteIconUrl"
                      :src="siteIconUrl"
                      :alt="setting.title"
                    ...
                    />
                    <Icon v-else icon="mingcute:image-line" width="20" height="20"/>
                  </div>
                  <input
                    ref="siteIconInputRef"
                    class="site-icon-input"
                    type="file"
                    :accept="siteIconAccept"
                    @change="onSiteIconChange"
                  />
                  <el-button
                    class="opt-button"
                    size="small"
                    type="primary"
                    :loading="siteIconUploading"
                    @click="openSiteIconPicker"
                  >
                    <Icon icon="mingcute:upload-2-line" width="16" height="16"/>
                  </el-button>
                  <el-button
                    v-if="setting.siteIcon"
                    class="opt-button"
                    size="small"
                    type="danger"
                    plain
                    :loading="siteIconUploading"
                    @click="removeSiteIcon"
                  >
...
import {settingQuery, settingSet, getGlobalToken, setGlobalTokenEnabled, generateGlobalToken, setSiteIcon, deleteSiteIcon} from "@/request/setting.js";
...
import {applySiteBranding, getSiteIconUrl} from "@/utils/branding.js";
...
const superAdmin = computed(() => userStore.user?.type === 0)
const siteIconInputRef = ref(null)
const siteIconUploading = ref(false)
const siteIconAccept = 'image/png,image/jpeg,image/webp,image/x-icon,image/vnd.microsoft.icon,image/svg+xml'
const siteIconUrl = computed(() => setting.value?.siteIcon ? getSiteIconUrl(setting.value.siteIcon) : '')
const editTitle = ref('')
...
function getSettings() {
  settingQuery().then(settingData => {
    ...
    settingStore.settings = settingData
    settingStore.domainList = settingData.domainList;
    applySiteBranding(settingData)
...
function openSiteIconPicker() {
  siteIconInputRef.value?.click()
}

async function onSiteIconChange(event) {
  ...
  if (!file.type.startsWith('image/')) {
    ElMessage.error(t('siteIconTypeError'))
    return
  }

  if (file.size > 2 * 1024 * 1024) {
    ElMessage.error(t('siteIconSizeError'))
    return
  }

  siteIconUploading.value = true
  try {
    const siteIcon = await fileToBase64(file, true)
    await setSiteIcon(siteIcon)
    await getSettings()
...
async function removeSiteIcon() {
  try {
    await ElMessageBox.confirm(t('delSiteIconConfirm'), t('warning'), {
      type: 'warning',
